...
<br/>
[![Tests](https://img.shields.io/badge/tests-999_passed-brightgreen.svg)](#开发)
[![MCP Tools](https://img.shields.io/badge/MCP_Tools-19-purple.svg)](#mcp-服务器)
[![Data Sources](https://img.shields.io/badge/Data_Sources-7-orange.svg)](#数据源)
[![Skills](https://img.shields.io/badge/Skills-7-red.svg)](#claude-code-技能)
[![REST Endpoints](https://img.shields.io/badge/REST_Endpoints-19-blue.svg)](#rest-api-与仪表盘)
[![Monitored Targets](https://img.shields.io/badge/Monitored_Targets-337+-teal.svg)](#1-多源异步采集引擎)

[摘要](#摘要) · [问题陈述](#问题陈述) · [形式化框架](#形式化框架) · [系统架构](#系统架构) · [核心创新](#核心创新) · [快速开始](#快速开始) · [CLI 参考](#cli-参考) · [REST API 与仪表盘](#rest-api-与仪表盘) · [MCP 服务器](#mcp-服务器) · [Claude Code 技能](#claude-code-技能) · [数据源](#数据源) · [生态系统](#生态系统) · [参考文献](#参考文献)
...
系统构建「**采集 -> 分析 -> 交叉关联 -> 异常检测 -> 告警分发**」的自动化情报管线，提供竞品矩阵、数据集谱系、组织关系图谱三维交叉分析能力，并以 Agent 原生方式暴露 MCP 19 工具 + REST 19 端点 + Skills 7 命令的完整接口层。
...
|:---|:---|:---|:---|
| **信息不对称** | 竞争对手的数据发布分散于 HF / GitHub / 博客 / 论文 / 社交媒体，无单一视图 | RSS 订阅覆盖率 < 30%，手动浏览效率 $O(n)$ | 7 源 337+ 目标统一采集，aiohttp 全链路并发 |
| **源碎片化** | 同一组织在不同平台发布不同粒度的信息，缺乏交叉关联 | 各平台独立监控，组织-数据集-论文关系断裂 | 竞品矩阵 + 数据集谱系 + 组织关系图谱三维交叉分析 |
| **被动式监控** | 依赖人工定期检查，异常变化（突发大量发布、竞品异动）无法实时感知 | 日报/周报模式，延迟 1-7 天 | 7 条异常检测规则 × 4 类别，Email + Webhook 自动推送 |
| **增量效率** | 全量扫描 API 配额消耗与总数据量成正比，无法提升至小时级频率 | 每次全量拉取，调用量 $\propto N$ | 组织级水位线增量扫描，调用量 $\propto \Delta N$ |
...
| 层 | 模块 | 职责 |
|:---|:---|:---|
| **采集层** | Trackers · Watermark Manager | 7 源异步采集，组织级水位线增量扫描，Playwright 动态渲染 |
| **分析层** | Classifiers · Trend Engine · Matrix Builder | 数据集分类、时序趋势计算、竞品矩阵构建 |
| **交叉分析层** | Lineage · Org Graph · Competitive Matrix | 数据集谱系追踪、组织关系图谱、三维交叉关联 |
| **检测层** | Anomaly Rules · Alert Engine | 7 条规则 × 4 类别异常检测，指纹去重，Email/Webhook 分发 |
| **持久层** | Time-Series Store · SQLite Snapshots | 批量 upsert + 作用域趋势计算，每日快照 |
| **接口层** | REST API · MCP Server · Skills · Dashboard | 19 + 19 + 7 Agent 接口 + 12 面板 Web 仪表盘 |
| **情报层** | AI Insights · DataRecipe Integration | LLM 分析报告生成，DataRecipe 逆向分析联动 |
...
| 分析维度 | 说明 | 输出 |
|:---|:---|:---|
| **竞品矩阵** | 组织 × 能力维度的对比表，识别差异化定位 | 结构化 JSON + Markdown |
| **数据集谱系** | 追踪数据集的派生关系链（fork / remix / extend） | DAG 图 + 链路分析 |
| **组织关系图谱** | 基于共同数据集、引用关系的组织协作网络 | Force-directed 图 |
...
Radar 以 Agent 原生方式暴露三套完整接口，覆盖从自动化采集到交互式分析的全工作流：
...
graph LR
    ...
    Recipe --> Synth["Synth<br/>合成"]
    Recipe --> Label["Label<br/>标注"]
    Synth --> Check["Check<br/>质量"]
    Label --> Check
    Check --> Audit["Audit<br/>审计"]
    Crew["Crew<br/>协商引擎"]
