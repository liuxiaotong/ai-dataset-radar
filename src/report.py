...
class ValueReportGenerator:
    ...
    def generate_report(
        ...
        lines.extend(
            [
                "",
                "---",
                "> Report generated by AI Dataset Radar v3 — High-Value Dataset Discovery System",
            ]
        )
    ...
    def _generate_top_datasets_section(self, scored_datasets: list[dict]) -> list[str]:
        ...
        for i, ds in enumerate(scored_datasets[:10], 1):
            ...
            cite_str = f"{cite_growth:.1f}/月" if cite_growth else "-"
    ...
    def _generate_domain_section(self, domain_breakdown: dict) -> list[str]:
        ...
        domain_labels = {
            ...
            "nlp": "LLM/NLP 语言模型",
            "vision": "Vision/Multimodal 视觉多模态",
            "code": "Code Generation 代码生成",
            "rlhf": "RLHF/Alignment 对齐",
        }
    ...
    def _generate_sota_section(self, sota_results: dict) -> list[str]:
        ...
        if ranked:
            lines.append("| 数据集 | SOTA模型数 | 覆盖领域 | 代表模型 |")
            lines.append("|--------|------------|----------|----------|")
    ...
    def _generate_citation_section(self, citation_data: list[dict]) -> list[str]:
        ...
        high_growth = [p for p in citation_data if p.get("citation_monthly_growth", 0) >= 10][:10]

        if high_growth:
            lines.append("| 论文/数据集 | 当前引用 | 月增长 | 增长率 |")
            lines.append("|-------------|----------|--------|--------|")
            ...
            for paper in high_growth:
                ...
                rate = (
                    f"{(growth / max(citations - growth, 1) * 100):.1f}%"
                    if citations > growth
                    else "N/A"
                )
    ...
    def _generate_model_usage_section(self, model_card_results: dict) -> list[str]:
        ...
        if valuable:
            lines.append("| 数据集 | 使用模型数 | 累计下载量 | 代表模型 |")
            lines.append("|--------|------------|------------|----------|")
    ...
    def _generate_papers_section(self, scored_datasets: list[dict]) -> list[str]:
        ...
        papers_with_data = [
            ds
            for ds in scored_datasets
            if ds.get("signals", {}).get("has_paper") and ds.get("total_score", 0) >= 30
        ][:10]

        if papers_with_data:
            lines.append("| 数据集/论文 | 评分 | 机构 | 链接 |")
            lines.append("|-------------|------|------|------|")
    ...
    def _generate_summary_section(
        ...
        lines.append(f"- **分析数据集总数:** {total} 个")
        lines.append(f"- **高价值数据集 (≥60分):** {high_value} 个")
        lines.append(f"- **中等价值数据集 (40-59分):** {medium_value} 个")
        lines.append(f"- **顶级机构数据集:** {top_inst} 个")

        if sota_results:
            sota_count = sota_results.get("unique_datasets", 0)
            lines.append(f"- **SOTA 关联数据集:** {sota_count} 个")

        if citation_data:
            high_cite = len([p for p in citation_data if p.get("citation_monthly_growth", 0) >= 10])
            lines.append(f"- **高引用增长论文:** {high_cite} 篇")

        if model_card_results:
            model_count = model_card_results.get("models_analyzed", 0)
            lines.append(f"- **分析模型数量:** {model_count} 个")
